...
from hei_fastapi_ddd.contexts.auth.api.auth_schemas import (
...
from hei_fastapi_ddd.contexts.auth.application.dto import LoginPayload, RegisterCommand
...
def _auth_options_response(account_type: AccountType) -> AuthOptionsResponse:
    ...
    from hei_fastapi_ddd.contexts.auth.api.oauth_schemas import (
...
async def auth_refresh(
    ...
    return success(await build_auth_service(db).refresh_session(session.token, account_type))
...
async def portal_register_send_code(
    ...
    """发送门户注册通道（邮箱/手机）验证码。"""
    await verify_captcha(payload.captcha_id, payload.captcha_value)
    await build_auth_service(db).send_register_code(
        channel=payload.channel,
        target=payload.target,
    )
    return success()
...
async def portal_register(
    ...
    password = await decrypt_password(payload.password_key_id, payload.password)
    return success(
        await build_auth_service(db).register_portal(
